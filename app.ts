...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    if (!ai) {
      ...
      let reply = "Hello! I am ShehriAwaz Civic Guide. I can help guide you on which department is responsible for civic issues in Pakistan.\n\nYou can ask me about:\n• Water supply or sewage issues (WASA)\n• Garbage collection or sanitation (LWMC/SSWMB)\n• Power outages or exposed wires (LESCO/IESCO/K-Electric)\n• Road damage or potholes (C&W/LDA/CDA)\n• Parks, green spaces, or encroachments (PHA/TMA)\n• Consumer rights and price complaints (DC Office)\n• General civic complaints (Citizen Portal)";

      if (lower.includes("water") || lower.includes("wasa") || lower.includes("sewage") || lower.includes("drain") || lower.includes("pipe")) {
        reply = `**Water Supply & Sewerage Issues in ${userCity || "your city"}:**\n\n• **Responsible Department:** WASA (Water and Sanitation Agency) — or KWSC in Karachi, WSSP in Peshawar, CDA Water Wing in Islamabad.\n• **What they handle:** Burst water pipes, low pressure, sewage overflow, drainage blockages, contaminated water supply.\n• **Helpline:** Call **1334** for WASA complaints across Punjab. Karachi: **1919** (KWSC). Peshawar: **091-9212345**.\n• **Resolution Time:** 24–48 hours for main pipeline leaks; 3–5 days for secondary drainage issues.\n• **Official Filing:** Use the Pakistan Citizen Portal app or call **1050** for government tracking.`;
      } else if (lower.includes("garbage") || lower.includes("trash") || lower.includes("clean") || lower.includes("waste") || lower.includes("dump")) {
        reply = `**Garbage & Sanitation Issues in ${userCity || "your area"}:**\n\n• **Responsible Department:** LWMC (Lahore), SSWMB (Karachi), RWMC (Rawalpindi), FWMC (Faisalabad), MWMC (Multan), or your local Waste Management Company.\n• **What they handle:** Uncollected garbage, open dumping, overflowing bins, blocked drains from waste, hazardous waste in public areas.\n• **Helpline:** LWMC: **1139**. For other cities, contact your local TMA or waste management company.\n• **Resolution Time:** Municipal waste is collected on schedule; backlog clearance usually takes 24–48 hours after community reporting.\n• **Tip:** Report open dumping spots to prevent them from becoming permanent illegal dump sites.`;
      } else if (lower.includes("electric") || lower.includes("kelectric") || lower.includes("power") || lower.includes("wire") || lower.includes("bijli") || lower.includes("transformer")) {
        reply = `**Electricity & Power Issues in ${userCity || "your city"}:**\n\n• **Responsible Authority:** Your regional DISCO — LESCO (Lahore), IESCO (Islamabad/Rawalpindi), K-Electric (Karachi), MEPCO (Multan), PESCO (Peshawar), FESCO (Faisalabad), GEPCO (Gujranwala), QESCO (Quetta).\n• **What they handle:** Power outages, load shedding beyond schedule, exposed wiring, transformer issues, new connections, billing disputes.\n• **Helpline:** Call **118** for emergency electricity complaints across all DISCOs.\n• **⚠️ Safety Warning:** If you see exposed live wires or sparking transformers, stay at least 20 feet away and call 118 immediately.\n• **Official Filing:** Use the Pakistan Citizen Portal app (call **1050**) to track your complaint with the DISCO.`;
      } else if (lower.includes("road") || lower.includes("pothole") || lower.includes("street") || lower.includes("bridge") || lower.includes("sadak")) {
        reply = `**Road & Infrastructure Issues in ${userCity || "your city"}:**\n\n• **Responsible Department:** C&W (Communication and Works) for provincial highways. TEPA/LDA (Lahore), KMC (Karachi), CDA (Islamabad), RDA (Rawalpindi), PDA (Peshawar), FDA (Faisalabad), MDA (Multan) for urban roads. Your local TMA for residential streets.\n• **What they handle:** Potholes, road damage, broken guardrails, collapsed pavements, damaged bridges, non-functional traffic signals.\n• **Process:** Road maintenance is scheduled quarterly; hazardous potholes are patched on priority.\n• **Helpline:** For urban roads, contact your local development authority. For highways, call the National Highways & Motorway Police: **130**.`;
      } else if (lower.includes("park") || lower.includes("green") || lower.includes("tree") || lower.includes("encroach")) {
        reply = `**Parks & Green Spaces in ${userCity || "your city"}:**\n\n• **Responsible Department:** PHA (Parks and Horticulture Authority) in Punjab cities. CDA Parks Wing in Islamabad. KMC Parks Department in Karachi.\n• **What they handle:** Park maintenance, tree trimming, green belt upkeep, playground equipment repair, encroachment on public green spaces.\n• **Helpline:** Contact your local PHA office or use the Pakistan Citizen Portal (call **1050**).\n• **Tip:** If you see unauthorized construction on public land or parks, report it immediately — encroachment is a criminal offense.`;
      } else if (lower.includes("consumer") || lower.includes("price") || lower.includes("overcharge") || lower.includes("shop") || lower.includes("market")) {
        reply = `**Consumer Rights & Price Complaints:**\n\n• **Responsible Authority:** District Commissioner (DC) Office — Price Control Section. Punjab Consumer Protection Courts.\n• **What they handle:** Price gouging, overcharging, fake products, unfair trade practices, violation of official price lists.\n• **How to File:** Visit your local DC Office or call the Punjab Consumer Protection Helpline: **0800-23456** (toll-free).\n• **Tip:** Keep your receipt as evidence. Official price lists must be displayed at shops — if they're not, that's a violation you can report.`;
      } else if (lower.includes("citizen portal") || lower.includes("1050") || lower.includes("khidmat")) {
        reply = `**Pakistan Citizen Portal (شہری پورٹل):**\n\n• **What it is:** The government's official complaint tracking system for all civic issues across Pakistan.\n• **How to Use:** Download the "Citizen Portal" app from the Play Store / App Store, or call **1050**.\n• **What You Can Report:** Any civic issue — water, electricity, roads, garbage, healthcare, education, police, and more.\n• **How It Works:** You submit a complaint → it's routed to the relevant department → you get tracking updates → the department must respond within the deadline.\n• **Tip:** File on ShehriAwaz first for community visibility, then also file on Citizen Portal for official government tracking.`;
      } else if (lower.includes("hello") || lower.includes("hi") || lower.includes("help") || lower.includes("kya")) {
        reply = `**Welcome to ShehriAwaz Civic Guide!** (شہری آواز)\n\nI can help you understand which government department handles your civic issue. Ask me about:\n\n• **Water/Sewage** — burst pipes, low pressure, sewage overflow\n• **Garbage/Sanitation** — uncollected waste, open dumping\n• **Electricity** — power outages, exposed wires, load shedding\n• **Roads** — potholes, road damage, broken infrastructure\n• **Parks/Green Spaces** — park maintenance, encroachments\n• **Consumer Rights** — price complaints, overcharging\n• **Citizen Portal** — how to file official complaints\n\nJust describe your problem and I'll guide you to the right department!`;
      }
